...
public class AnonymousResponseCommand extends Command {

    public AnonymousResponseCommand(Modmail modmail) {
        super(modmail, modmail.getLang().get("command_anonymous_response"), 0, -1);
        setUsage(modmail.getLang().get("usage_anonymous_response"));
    }
    ...
    public CommandResult execute(Message message, String[] args) {
        ...
        try {
            member = modmail.getMainGuild().retrieveMemberById(targetID).complete();
